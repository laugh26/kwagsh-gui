...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("kwagsh://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "kwagsh:");
    }
    if(!_request.startsWith("kwagsh:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with kwagsh:"), QtCriticalMsg));
      return;
    }
