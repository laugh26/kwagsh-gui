...
void ConfirmSendDialog::showPasymentDetails(quint64 _total) {
    setWindowTitle(QString(tr("Confirm sending %1 KGH")).arg(CurrencyAdapter::instance().formatAmount(_total)));
...
void ConfirmSendDialog::confirmNoPaymentId() {
    m_ui->m_paymentIdLabel->setText(QString(tr("<html><head/><body><p>Are you sure you want to send kwagsh<strong>without Payment ID</strong>?</p></body></html>")));
}
